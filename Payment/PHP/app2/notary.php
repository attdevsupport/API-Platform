...
"\"SubscriptionRecurringNumber\": 5,\n \"SubscriptionRecurringPeriod\" : \"MONTHLY\",\n \"SubscriptionRecurringPeriodAmount\" : 1, }";
    ...
    if($signedPayload==null || $signedPayload == '')
        $signedPayload = $_SESSION["pay2_signedPayload"];
    if($signedPayload==null || $signedPayload == '')
    ...
    if($signature==null || $signature =='')
        $signature = $_SESSION["pay2_signature"];
    if($signature==null || $signature == '')
...
if($signPayload!=null) {
  ...
  if($responseCode==200) {
    ...
    $_SESSION["pay2_signedPayload"]=$signedPayload;
    $signature = $jsonResponse->{"Signature"};
    $_SESSION["pay2_signature"] =  $signature;
    if($_REQUEST["return"]!=null){
      header("location:subscription.php?signedPayload=".$signedPayload."&signature=".$signature);
    }
  } else {

...
8) FootBallFilePath			: {set the value to the path of the file, which application creates and stores the football vote count }

9) BaseBallFilePath			: {set the value to the path of the file, which application creates and stores the baseball vote count }

10) BasketBallFilePath		: {set the value to the path of the file, which application creates and stores the basketball vote count }
...
Suppose you copied the sample app files in your IIS server webroot/sms/app2/ folder, In order to run the sample application, type in'http://IIS_HOSTNAME/sms/app2/Default.aspx'
